...
class KnowledgeManager: ObservableObject {
    ...
    func searchKnowledge(
        ...
        do {
            // 3. 执行智能搜索（使用文本搜索）
            let dictResults = try await SupabaseManager.shared.searchKnowledgeWithTextSearch(
                query: query,
                limit: limit
            )
            
            // 转换为KnowledgeItem
            let results = dictResults.compactMap { dict -> KnowledgeItem? in
